...
class Geocoding {
	...
	public function get_latitude() {//this will return the latitude or horizontal line == y position
		...
		return $this->google_response['results'][0]['geometry']['location']['lat'];
	}

	public function get_longitude() {//this will return the longitude or vertical line == x position

		return $this->google_response['results'][0]['geometry']['location']['lng'];
...
/******* PRIVATE FUNCTIONS *******/
	...
	private function make_request($address) {
		...
		$raw_google_response = file_get_contents($link);//google response--this is raw data
		$this->google_response = json_decode($raw_google_response, true);//--json decoded into an object!
