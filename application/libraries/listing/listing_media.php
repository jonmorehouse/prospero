...
/********** PUBLIC FUNCTIONS ***********/
	...
	public function slideshow_images() {//will return the slideshow for the content page
		...
		$urls = array();//will be an array of urls!

		// for each individual slideshow image, make sure that you give an alt tag and url for this!
		foreach ($this->images as $image_id) {

			// url is database driven based upon image_id
			$url = $this->CI->media->get_url('slideshow_image', $image_id);

			// alt is the name of the property
			$alt = $this->CI->format->word_format($this->CI->general->get_category($this->property_id, "name"));

			// push the image array into the urls to be returned
			array_push($urls, array('url' => $url, 'alt' => $alt));
		}
