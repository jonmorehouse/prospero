...
class Browse{
	...
	public function browse_header($id, $category = false, $filter = false) {
		...
		// GET THE ADDITIONAL HEADERS THAT ARE CRITERIA SPECIFIC
		if($category and $filter)//if we specify actual categories and filters
			$specific_header = $this->CI->information->header_information($category, $filter);//add the break here so that we don't accidentally include it in the page_title
		
		// WE ONLY ADD TO THE HEADER IF THE SPECIFIC HEADER EXISTS
		if(isset($specific_header))	
			$header = $header . "{$this->CI->format->word_format($specific_header)}";
