...
*/
Project.Modules.gallery = function (images, container, template) {
	...
	function start() {//this is a recursively running function that controls itself as the animation plays!
		...
		config.counter = (config.counter < config.total_images) ? (config.counter++) : (config.counter = 1);
		...
		if (config.playing) return setTimeout(start(), config.interval);
